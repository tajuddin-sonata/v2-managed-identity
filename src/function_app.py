...
def main(req: func.HttpRequest, context: func.Context) -> func.HttpResponse:
    ...
    try:
        ...
        # return func.HttpResponse("Media file not found on bucket.", status_code=404)
    ...
    try:
        ...
        # return func.HttpResponse("ffprobe failed.", status_code=500) ##########################
    ...
    if media_info["audio_streams"] == 0 or media_info["audio_channels"] == 0:
        with TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                logging.error(dumps({"ffprobe_error": str(e.stderr, "utf-8")}, indent=2))
                logging.error(f"Error injecting blank audio into video transcoding: {str(e)}")  #############
                # return func.HttpResponse("injecting blank audio into video transcoding failed.", status_code=500) #####################
        ...
        ### Check that a new version/etag of the media_blob exists now
    ...
    if temp_audio_format_name not in media_info["file_format"].split(","):
        with TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                logging.error(dumps({"ffprobe_error": str(e.stderr, "utf-8")}, indent=2))
                return func.HttpResponse("temp audio wav transcoding failed.", status_code=500)
            ...
            if media_info["video_streams"] > 0:
                ...
                if not temp_audio_blob.exists():
                    # return func.HttpResponse("temp audio failed to upload to bucket.", status_code=500) ######### 
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            # return func.HttpResponse("waveform generation failed.", status_code=500) ##########
    ...
    if not waveform_file_blob.exists():
        # return func.HttpResponse("waveform failed to upload to bucket.", status_code=500) #############
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            # error_description = f"{compressed_audio_extension_type.replace('.', '')} transcoding failed"
            # return func.HttpResponse(error_description, status_code=500)
    ...
    if not compressed_audio_blob.exists():
        # return func.HttpResponse("compressed audio failed to upload to bucket.", status_code=500) ##############
