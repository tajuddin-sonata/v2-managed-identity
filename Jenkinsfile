...
pipeline {
    ...
    parameters{
        ...
        string(name: 'AZURE_FUNCTION_APP_NAME', defaultValue:'ssna-func-cca-dev-eus-wfredact', description: '''The name of FunctionApp to deploy
            ...
            ssna-func-cca-prod-eus-wfredact   for production env.
            ''' )
        /*
                ...
                script {
                    ...
                    sh """
                        ...
                        if [ -z "$ver" ]; then
                            ...
                            cd src
                            zip -r "../az-ci-redact-\${artifact_version}.zip" *
                            cd $WORKSPACE
                            echo "CREATED [az-ci-redact-\${artifact_version}.zip]"
                            curl -v -u deployment:deployment123 --upload-file \
                                "az-ci-redact-\${artifact_version}.zip" \
                                "http://74.225.187.237:8081/repository/packages/cca/ci-config-service/az-ci-redact-\${artifact_version}.zip"
                        else
                            artifact_version=$ver
                            echo "Downloading specified artifact version from Nexus..."
                            curl -v -u deployment:deployment123 -O "http://74.225.187.237:8081/repository/packages/cca/ci-config-service/az-ci-redact-\${artifact_version}.zip"
                        fi
                        rm -rf "az-ci-redact-\${artifact_version}"
                        unzip "az-ci-redact-\${artifact_version}.zip" -d "az-ci-redact-\${artifact_version}"

                        ls -ltr
                        cd az-ci-redact-\${artifact_version}
                        ls -ltr
