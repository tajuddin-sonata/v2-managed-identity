...
pipeline {
    ...
    parameters{
        ...
        string(name: 'AZURE_FUNCTION_NAME', defaultValue:'dev-func-wftranscode-sitl-eus', description: '''The name of FunctionApp to deploy
            dev-func-wftranscode-sitl-eus
            stg-func-wftranscode-sitl-eus
            prod-func-wftranscode-sitl-eus
            ''' )
                    ...
                    sh """
                        ...
                        if [ -z "$ver" ]; then
                            ...
                            curl -v -u nexus-user:nexus@123 --upload-file \
                                ...
                                "http://74.225.187.237:8081/repository/packages/cca/$FILE_PREFIX-ci-transcode-\${artifact_version}.zip"
                        else
                            artifact_version=$ver
                            echo "Downloading specified artifact version from Nexus..."
                            curl -v -u nexus-user:nexus@123 -O "http://74.225.187.237:8081/repository/packages/cca/$FILE_PREFIX-ci-transcode-\${artifact_version}.zip"
                        fi
