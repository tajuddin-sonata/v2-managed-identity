...
pipeline {
    ...
    parameters{
        ...
        string(name: 'AZURE_FUNCTION_NAME', defaultValue:'dev-wf-transcribe', description: '''The name of FunctionApp to deploy
            dev-wf-transcribe 
            stg-wf-transcribe 
            prod-wf-transcribe ''' )
        ...
        string(name: 'AZURE_FUNCTION_ASP_NAME', defaultValue:'jenkins-ASP', description: '''The name of App service Plan for FunctionApp to deploy
            v2-functions-ASP
            jenkins-ASP
            dev-wf-transcribe-ASP 
            stg-wf-transcribe-ASP
            prod-wf-transcribe-ASP''' )
                    ...
                    script {
                        sh """
                            ...
                            ${SONARQUBE_SCANNER_HOME}/bin/sonar-scanner \
                                -Dsonar.projectKey=My-Transcribe-Project \
                                -Dsonar.host.url=http://4.240.69.23:9000 \
                                -Dsonar.sources=src \
                                -Dsonar.sourceEncoding=UTF-8 \
                                -Dsonar.python.version=3.11
                                -Dsonar.login=sqp_76718481b26452900254b93f8dca7633df356737
                        """
                    }
                }
            }
